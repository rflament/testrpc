...
var Interface = {
  server: function(options, callback) {
    ...
    this.provider(options, function (err, provider) {
  ...
  provider: function(options, callback) {
    ...
    if (options.useMongo) {
      options.accounts = [];
      require('../lib/model/mongoose_init');
      var mongoURL = 'localhost/testrpc';
      ...
      mongoose.connect(mongoURL, function (err, res) {
        if (err) {
          ...
          Account.find({}).then(
            ...
            function (accounts) {
              console.log('found ' + accounts.length + ' accounts in mongo');
              console.log(accounts);
              options.accounts = accounts;

              mongoose.disconnect();
              self.initialize(options, callback);
            });
        }
      });

    }
    else this.initialize(options, callback);
